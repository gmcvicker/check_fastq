...
static char *get_prefix(const char *output_dir, char *input_path) {
  int len, n, i;
  char *prefix, *input_filename;

  len = strlen(input_path);

  /* only take filename portion of input file */
  for(i = len-1; i >= 0; i--) {
    if(input_path[i] == '/') {
      break;
    }
  }
  input_filename = &input_path[i+1];
  len = strlen(input_filename);

  fprintf(stderr, "input_filename='%s'\n", input_filename);

  if(util_str_ends_with(output_dir, "/")) {
    prefix = util_str_concat(output_dir, input_filename, NULL);
  } else {
    prefix = util_str_concat(output_dir, "/", input_filename, NULL);
  }

  /* truncate prefix by removing .gz, .txt or txt.gz" */
  len = strlen(prefix);
  if(util_str_ends_with(prefix, ".txt.gz")) {
    n = strlen(".txt.gz");
    prefix[len - n] = '\0';
  }
  else if (util_str_ends_with(prefix, ".gz")) {
    n = strlen(".gz");
    prefix[len - n]= '\0';
  }
  else if(util_str_ends_with(prefix, ".txt")) {
    n = strlen(".txt");
    prefix[len - n] = '\0';
  }
